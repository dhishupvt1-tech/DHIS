...
export async function addEventor(
  eventor: Omit<Eventor, "id" | "createdAt" | "isActive">,
  ownerUserId: string
) {
  const result = await prisma.eventor.create({
    data: {
      ...
      eventorsUsers: {
        create: [{ userId: ownerUserId, role: UserRole.OWNER }],
      },
    },
  });
...
export async function updateEventor(eventor: Omit<Eventor, "createdAt">) {
  await prismaQueryWrapper(() =>
    prisma.eventor.update({ where: { id: eventor.id }, data: eventor })
  );
}
